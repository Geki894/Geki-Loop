...
const e=s=>String(s??'—').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));async function render(){const d=await fetch('/api/state').then(r=>r.json());const s=d.state||{};document.getElementById('app').innerHTML='<div class="grid"><div class="card"><div class="muted">Phase</div><div class="value">'+e(s.phase)+'</div><p>Status: '+e(s.status)+'</p></div><div class="card"><div class="muted">Current work</div><div class="value">'+e(s.currentEpic||s.currentStory)+'</div><p>Gate: '+e(s.currentGate)+'</p></div><div class="card"><div class="muted">GitHub</div><div class="value">'+(d.github.available?'Connected':'Unavailable')+'</div><p>Open PRs: '+e(d.github.pullRequests?.length||0)+'</p></div></div><div class="card"><h2>Modules</h2>'+Object.keys(d.lock.modules||{}).map(x=>'<span class="pill">'+e(x)+'</span>').join('')+'</div><div class="card"><h2>Recent events</h2><table><tr><th>Time</th><th>Event</th></tr>'+d.events.map(x=>'<tr><td>'+e(x.at)+'</td><td><code>'+e(x.type)+'</code></td></tr>').join('')+'</table></div>'}render();setInterval(render,5000);
</script></body></html>`;
