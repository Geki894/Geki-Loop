...
1. Read `.geki/config.json`, `.geki/state/planning.json`, `.geki/state/current-run.json`, `.geki/architecture.json`, `.geki/lock.json`, and `.geki/handoff/current.yaml` when present.
...
4. State: delivery profile, current phase/stage, completed milestone, active delivery slice, review round, blocker, and exactly one recommended next command/skill.
